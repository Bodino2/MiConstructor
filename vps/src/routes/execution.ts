...
export function executionRouter(database: Database) {
  ...
  router.post("/projects/:id/contracts/accept", requireAuth, requireRole("cliente"), async (request, response, next) => {
    try {
      ...
      const result = await withTransaction(database, async (client) => {
        ...
        for (let i = 0; i < parsed.data.milestones.length; i++) {
          const item = parsed.data.milestones[i];
          const milestoneId = randomUUID();
          await client.query(`INSERT INTO milestones (id, project_id, position, title, description, amount_cents, due_date)
                              VALUES ($1,$2,$3,$4,$5,$6,$7)`, [milestoneId, projectId.data, i + 1, item.title, item.description, item.amountCents, item.dueDate ?? null]);
          await client.query(`INSERT INTO work_passport_entries (project_id, actor_user_id, event_type, entity_type, entity_id, summary, metadata)
                              VALUES ($1,$2,'HITO_CREADO','milestone',$3,$4,$5::jsonb)`, [projectId.data, request.user!.id, milestoneId, item.title, JSON.stringify({ position: i + 1, amountCents: item.amountCents })]);
        }
        await client.query(`INSERT INTO work_passport_entries (project_id, actor_user_id, event_type, entity_type, entity_id, summary, metadata)
  ...
  router.get("/projects/:id/contract/pdf", requireAuth, async (request, response, next) => {
    try {
      const projectId = z.string().uuid().safeParse(request.params.id);
      if (!projectId.success) return response.status(400).json({ error: "Proyecto no válido." });
      const contractResult = await database.query<any>(`SELECT c.*, cu.name AS client_name, pu.name AS professional_name, pu.company_name
      ...
      const milestones = await database.query<any>("SELECT position, title, description, amount_cents, due_date FROM milestones WHERE project_id = $1 ORDER BY position", [projectId.data]);
      ...
      const pdf = buildTextPdf(`MiConstructor - Contrato de obra ${contract.id}`, [
        ...
        { heading: "Hitos", body: milestones.rows.flatMap((m: any) => [`${m.position}. ${m.title} - ${euro(m.amount_cents)}${m.due_date ? ` - ${String(m.due_date).slice(0,10)}` : ""}`, m.description || "Sin descripción adicional."]) },
  ...
  router.post("/milestones/:id/approve", requireAuth, requireRole("cliente"), async (request, response, next) => {
    try {
      ...
      const result = await withTransaction(database, async (client) => {
        ...
        const pending = Number((await client.query("SELECT count(*) AS count FROM milestones WHERE project_id=$1 AND status <> 'LIBERADO'", [row.project_id])).rows[0].count);
        if (pending === 0) {
  ...
  router.get("/projects/:id/messages", requireAuth, async (request, response, next) => {
    try {
      const projectId = z.string().uuid().safeParse(request.params.id);
      if (!projectId.success) return response.status(400).json({ error: "Proyecto no válido." });
      const conversation = (await database.query<any>("SELECT * FROM conversations WHERE project_id=$1 AND ($2='admin' OR client_id=$3 OR professional_id=$3)", [projectId.data, request.user!.role, request.user!.id])).rows[0];
  ...
  router.post("/projects/:id/messages", requireAuth, async (request, response, next) => {
    try {
      ...
      const conversation = (await database.query<any>("SELECT * FROM conversations WHERE project_id=$1 AND (client_id=$2 OR professional_id=$2)", [projectId.data, request.user!.id])).rows[0];
      if (!conversation) return response.status(404).json({ error: "Conversación no encontrada." });
      const id = randomUUID();
  ...
  router.post("/projects/:id/reviews", requireAuth, async (request, response, next) => {
    try {
      ...
      const contract = (await database.query<any>("SELECT c.*,p.status AS project_status FROM work_contracts c JOIN projects p ON p.id=c.project_id WHERE c.project_id=$1 AND (c.client_id=$2 OR c.professional_id=$2)", [projectId.data, request.user!.id])).rows[0];
      ...
      try {
        ...
        if (error?.code === '23505') return response.status(409).json({ error: "Ya has enviado tu reseña para este proyecto." });
        throw error;
      }
