...
export function buildTextPdf(title: string, sections: Array<{ heading: string; body: string[] }>) {
  ...
  for (let i = 0; i < pageObjectIds.length; i++) {
    ...
    if (pageObjectId === undefined || contentObjectId === undefined) throw new Error("Invalid PDF page structure");
    objects[pageObjectId - 1] = `<< /Type /Page /Parent ${pagesObjectId} 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 ${fontRegular} 0 R /F2 ${fontBold} 0 R >> >> /Contents ${contentObjectId} 0 R >>`;
  }
